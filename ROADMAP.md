...
- 每个节点会持久化最近一次运行的 ID、状态、耗时、输入来源端口、输出端口与错误摘要；尚未提供跨项目的完整运行历史、耗时聚合与可检索日志。
...
- 新增「迭代」控制节点（`iterate`）：输入 `in-list`（list.items@1），输出 `out-items`（list.items@1）。
  ...
  - 支持严格串行执行、失败策略（skip / fail / retry）、重试次数（maxRetries）、限数（limit）与取消。
...
- 当前完成标准：20 个镜头可按顺序、限数和失败策略受控批量执行；单项失败不丢失其它成功结果。并发与可控断点续跑尚未实现。

完成标准：

- ~~20 个镜头可以受控地批量执行。~~ ✅ 串行 + 限数
- ~~单项失败不会丢失其他成功结果。~~ ✅ skip 策略保留失败项，已完成项不受影响
- 中止后仅保留结果摘要；尚未提供“只继续未完成项”的控制入口。

后续可增强（非 R4 阻塞项）：

- 迭代体复杂化：目前 runSubflow 串联执行下游节点链；「迭代体内多分支 / 子进程组」留待进一步子图机制。
- R3 角色/场景/镜头清单 Schema 与 R4 列表协议结合，提供更细粒度元素 Schema。
- 并发执行需要先实现每个列表项独立的运行态，避免共享节点产物互相覆盖。
- 进度百分比与断点续跑 UI。
