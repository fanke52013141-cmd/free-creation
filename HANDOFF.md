...
4. **IA-2.3 运行中心（下一开发切片）**：跨节点查看运行历史；失败记录支持重试节点、查看输入和回到节点，重试必须继续走统一 executor，不得在 React 中直接调用模型；补“从资产打开对应运行记录”。
5. **IA-2.4 验收**：补运行中心、失败恢复和保存重开一致性测试，并在 Electron 中完成“生成图片 → 重复生成 → 切换结果 → 定位节点 → 查看运行记录 → 失败重试”人工路径。

本切片新增 `test/media-index.test.ts`，覆盖生成结果集合来源、当前/历史结果、稳定 `nodeId` 筛选、无运行记录资产和安全关键词；完整门禁通过，当前为 28 个测试文件、453 个用例。IA-2 仍保持本地单用户架构，不引入登录、团队、云端项目或权限系统；导演台真实 3D、图片转模型和供应商级批量候选不作为 IA-2 前置工作。P3.4 的真实桌面端批量媒体回归属于并行验收债务，不阻塞 IA-2 的索引与资产中心开发，但必须在发布前完成。详细借鉴差异与验收表见 [INFINITE_ATELIER_ADOPTION_PLAN.md](./docs/INFINITE_ATELIER_ADOPTION_PLAN.md)。
