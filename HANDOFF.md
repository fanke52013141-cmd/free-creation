...
- ⏳ “分镜→批量生图”模板可在 P3.3 的模板设计与 20–100 项桌面端回归完成后开放；当前不伪装为可恢复批量模板。
...
- P3.3：提供“分镜→批量生图”模板，并完成 20–100 项桌面端回归；可控并发仍需每项独立运行态，未完成隔离前不得开放并发配置。
...
2. 迭代节点已具备明确的 `out-item` 循环体边界、项级检查点和协作式暂停恢复，但仍严格串行，不支持可控并发。
