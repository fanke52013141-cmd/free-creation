...
- ⏳ “分镜→批量生图”模板可在 P3.2（进度、暂停/恢复和失败项重跑）完成后开放；当前不伪装为可恢复批量模板。
...
- P3.2：增加进度、暂停、取消、只重跑失败项、断点续跑；恢复时按稳定 item id 校验，防止列表变更后错误复用。
- P3.3：在每项独立运行记录的基础上设计可控并发；未完成隔离前不得开放并发配置。
...
2. 迭代节点已具备明确的 `out-item` 循环体边界与链式/分支数据依赖，但仍严格串行，不支持暂停恢复或可控并发。
