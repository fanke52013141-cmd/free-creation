...
- `npm run test`：25 个测试文件、427 项用例全部通过。
...
1. **IA-2.0 数据索引**：盘点 `MediaAsset`、`meta.nodeRun`、`meta.nodeResult` 和 tldraw 快照，建立只读资产/运行索引；只保留来源节点、端口、运行 ID、时间、状态和本地路径等安全字段。
2. **IA-2.1 资产筛选**：按图片/视频/音频/文件、来源节点、运行状态、时间和关键词筛选；默认按最新结果排序，补齐空状态、无结果和媒体缺失状态。
3. **IA-2.2 来源定位**：从资产详情定位到稳定 `nodeId`、对应运行记录和本地文件；节点已删除或记录损坏时明确提示且不修改项目数据。
4. **IA-2.3 运行中心**：跨节点查看运行历史；失败记录支持重试节点、查看输入和回到节点，重试必须继续走统一 executor，不得在 React 中直接调用模型。
5. **IA-2.4 验收**：补索引/筛选/定位/失败恢复自动化测试，并在 Electron 中完成“生成图片 → 重复生成 → 切换结果 → 定位节点 → 查看运行记录 → 失败重试”人工路径。

IA-2 仍保持本地单用户架构，不引入登录、团队、云端项目或权限系统；导演台真实 3D、图片转模型和供应商级批量候选不作为 IA-2 前置工作。P3.4 的真实桌面端批量媒体回归属于并行验收债务，不阻塞 IA-2 的索引与资产中心开发，但必须在发布前完成。详细借鉴差异与验收表见 [INFINITE_ATELIER_ADOPTION_PLAN.md](./docs/INFINITE_ATELIER_ADOPTION_PLAN.md)。
