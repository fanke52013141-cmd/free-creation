...
> 状态：强制规范 1.9（P0-P4 图片修改、P0-P3.1、M0-M4 本地图片/视频媒体处理、代码离线确定性、音频职责拆分与统一画布基础交互已落地）
...
16. 同一节点的输入端口中，同一种 `PortType` 至多出现一次；同一节点的输出端口中，同一种 `PortType` 也至多出现一次。多个同类输入必须合并为一个 `many` 端口；多个同类结果必须使用一个集合输出，或各自物化为关联的独立资产节点。不得为了“主图/参考图”“人声/伴奏”等显示目的并列相同颜色、相同类型的端口。唯一的受控例外是代码节点中用户声明的**具名函数参数**：它们必须有不同的稳定参数名和端口 ID，表达独立函数参数，不能充当通用素材入口。
...
| `iteration` | 循环临时作用域   | 仅由循环的当前项注入 JSON 输入，不是项目级输出 |
| `camera`    | 机位参数         | `previs.camera@1`；仅在机位通道间传递          |
...
- `iteration` 只可连接到 JSON 输入；`camera` 只可连接到同类机位输入。
...
- `out-item`（`iteration`，可选）：当前列表项的**临时作用域**数据。只能连接循环体的 JSON 入口输入端口；运行器会为每个 item 把该值注入这条端口，并沿循环体内的真实数据连线继续执行。
