...
9. 连线是可撤销的真实数据依赖：点选连线后按 `Delete/Backspace` 只删除该连线，不删除节点；删除节点时会级联清理它的悬空连线。
...
| 输入端口              | 类型                     | 语义                                                         |
| --------------------- | ------------------------ | ------------------------------------------------------------ |
| `in-image`            | image                    | 可选首帧。仅支持首尾帧模式的模型按 `first_frame` 提交。      |
| `in-last-image`       | image                    | 可选尾帧。仅在供应商支持硬首尾帧约束时可用。                 |
| `in-reference-images` | image many               | 多张参考图，连线保存顺序就是提示词中“图片 1、图片 2”的顺序。 |
| `in-reference-video`  | video（多值，最多 3 段） | 运动/镜头参考。                                              |
| `in-reference-audio`  | audio many               | 声音、节奏或音色参考，不能伪装为“生成音频”开关。             |

`@ 引用图片` 是以上 `in-reference-images` 的辅助交互：用户从画布选择一个真实
