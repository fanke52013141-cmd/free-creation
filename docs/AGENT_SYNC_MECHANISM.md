...
> 状态（2026-09-17）：**历史机制存档。** Agent 对接实现已随本轮下线移除（原因与重接前提见
> [AGENT_INTEGRATION_PLAN.md](./AGENT_INTEGRATION_PLAN.md)），本文描述的门禁测试与
> `agent:*` 脚本已不存在。重接 Agent 时按本文重建门禁，避免重建时重新踩坑。
>
> 版本：1.0（2026-09-03）
> 原状态：已落地——门禁测试与 CI 步骤均在 `main` 生效。
