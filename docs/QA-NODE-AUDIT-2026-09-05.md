...
- **代码级功能与契约回归：通过。** `npm run verify` 通过，67 个测试文件、852 个测试全部通过；类型检查和 Electron/Vite 构建通过。
- **节点契约覆盖：通过。** 当前生成的能力矩阵包含 23 个可暴露能力；MCP、CLI、Headless 标志和输入/输出快照由契约测试校验。
- **连线与并行路径：代码级通过。** 连线兼容矩阵 84 项通过，单节点、局部子图和一对多扇出运行均有回归测试；输出投影、拓扑和 Headless Runner 测试通过。
...
| `npm run test` | 67 files / 852 tests 全部通过 |
...
| `test/manual-run.test.ts` | 8 项通过：单节点、JSON 正文直跑、局部子图和一对多扇出 |
| `test/video-transform.test.ts` | 17 项通过：视频变换、人声分离及本地能力前置门禁 |
...
| 并行/局部运行 | 代码级一对多扇出、并行分支与局部运行测试通过；浏览器实测覆盖同画布的多节点链路，真实拖拽多分支仍建议在桌面端复核 |
...
### P1：JSON 正文编辑态已与运行入口统一

已将 JSON 编辑器对齐文本节点的 tldraw 编辑生命周期：进入编辑时登记 `editor.setEditingShape`，退出/卸载时清理编辑态，提交时从最新正文写回 `props.text`。新增手动运行回归覆盖“卡片正文 → `out-json`”路径，避免详情页测试成功而卡片运行失败的两套语义分叉。

仍需在桌面端复核一次“输入后立即点击运行”这一真实事件顺序；若后续发现浏览器/桌面事件时序差异，应优先补事件级测试，而不是再增加另一套 JSON 存储字段。
