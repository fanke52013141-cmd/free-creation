...
### 7.9 §8 P2/P3 逐节点操作矩阵：17 个免费节点真机全绿，查出七处「界面说可以、实际用不了」（2026-09-19 追加）

前面几节的真跑都是单点验收。这一节把 §8 P2 要求的
「新建 → 配置 → 单输入 → 多输入 → 运行 → 成功/失败 → 保存重载」做成一条可重复门禁：
`scripts/audit-node-operation-matrix.cjs`（`npm run test:node-matrix`；`MATRIX=text,json`
可只跑子集）。

- **全程免费**：不配供应商、不调任何模型；默认在 `%LOCALAPPDATA%\canvas-node-matrix-<时间戳>`
  建全新数据目录，绝不读写用户真实项目库。
- 每条配方开一个全新项目（画布天生空白、相机天生归位），节点用「从创建面板按住拖到画布指定
  位置松手」这个真实手势落位，所以端口坐标是确定的；缩放固定在 60%，靠 dock 的
  「重置缩放 / 缩小」按钮达成，不用内部 API。
- **界面与磁盘两条证据**：界面读 `.node-status` 的 aria-label、`.json-status` 徽标、
...
- 本轮覆盖 **17 个不依赖模型 Key 的节点**（文本 / JSON / 处理 / 分镜板 / 结构数据 / 循环 /
  ...
  人声分离 / 导演台），**161/161 全绿**，截图在 `artifacts/node-matrix-2026-09-19/`。
  媒体素材由 ffmpeg 现造真实字节（640×480 色条 PNG、4 秒带音轨 MP4），走画布拖拽导入；
  产物不只查数据库记录，还用 `ffprobe` 量真实时长、按字节确认文件真的落在磁盘上。
- **§8 P3 的取消 / 续跑 / 重启恢复也并进同一条门禁**（配方 `p3`，20 条断言，整跑 179/179）：
  4 项 × 每项 2 秒异步等待的循环体，跑满 4.5 秒后点顶栏「停止」——节点落定为
  `cancelled`、进度 `成功 2 · 跳过 2`、不记成功；切「续跑未完成」再跑，逐项状态是
  `["reused","reused","done","done"]`，即已成功的两项确实被复用而不是重跑；
  最后**关掉整个 Electron 进程再开一次**（不是重载窗口），4 条逐项结果、循环体的运行结果
  与进度条都从磁盘回来了。
...
1. **误点一次运行，节点就再也喂不进下游**（`renderer/src/engine/executor.ts`）。
   ...
   `projectNodeOutputs` 早就给正文/资产类节点（`outputSource: 'document'`）开了豁免——
   这类端口的值就是卡片当前内容，不存在"上一轮遗留值"。两处规则相反的实际后果：在一张空的
   文本节点上点一次运行（按钮可点，执行器回 `skipped`），之后它写好的正文连到下游就报
   「连线 … 的上游未产生 out-text 输出」，而界面上那张卡明明有字。现在 `skipped` 的正文类
   上游照常供值，`failed` 仍然拦住（原有那条防遗留测试继续通过）。测试：
   `test/manual-run.test.ts` 新增一条；变异验证：删掉豁免只让新测试变红、旧测试不受影响。
2. **循环节点的 `in-list` 没按契约矩阵标必填**（`nodes/specs/index.tsx`）。
   `NODE_COMPLIANCE_MATRIX.md` 记的是「必填」，代码里却是可选——没接列表时运行按钮照样能点，
   跑完只回一句 `skipped`。现在按钮置灰并显示「缺少输入：列表」，
   `test/node-contract-snapshot.test.ts` 把 `required === true` 钉住。
3. **端口圆点没有稳定标识**（`canvas/NodeCardView.tsx`）。圆点只有 `title="名称 · 类型"`，
...
7. **循环节点的进度条从来没出现过**（`shared/engine/executors/iterate.ts`）。
   执行器每跑完一项就 `publishProgress()` 写一次 `{items, progress}`，卡片也照着
   `result?.progress` 渲染进度条和「续跑 · 4/4 · 成功 4」这句——但 `parseIterateResult`
   回填时只取 `items`，把 `progress` 丢了，于是这段 UI 是**永远走不到的死分支**
   （批量跑到一半看不到进度，只能靠顶栏那条总进度）。现在进度随结果一起解析回来，
   旧记录里没有 progress 时仍然如实返回无进度。测试：`test/iterate.test.ts` 新增一条
   （含反向断言：只有 items 的旧记录不会凭空多出进度）；真机回执：重启后卡片文案为
   `续跑 · 4/4 · 成功 2 · 失败 0`。
...
- 卡片内容超过最高档位（440）时，底部工具条会落进卡片自己的内部滚动区——JSON 节点贴进
  两段结构化数据后，「编辑数据 / 格式化 / 复制」要先把卡片滚一下才点得到。门禁为此显式做了
  一次 `scrollIntoViewIfNeeded`；是否该给这类卡片留一个常驻工具条，属于呈现规范的决定。
- 每轮矩阵会抛 5 012 次未捕获的 `The source image cannot be decoded` 与 28 次
  `Failed to fetch`。落地页静置 6 秒、刚打开的空闲画布静置 6 秒都是 0 次，只在画布被操作时
  出现；打包产物里能找到 `https://cdn.tldraw.com`，即 tldraw 的默认字体/图片资源在离线且
  CSP 受限的桌面环境里反复取失败。不影响本矩阵任何断言，故单列一条、不混进
  「整轮没有未预期的前端异常」这条门禁（否则门禁永远红，盖住真正的新异常）。
  本地单用户软件反复联网失败值得单独收口：把 `assetUrls` 指到打包内资源或显式关掉。
...
1. ✅ 门禁已落地：`npm run test:node-matrix`（详见 §7.9）；
2. ✅ 17 个不依赖模型 Key 的节点全部跑完（结构化 7 个 + 媒体族 9 个 + 导演台），
   再加 P3 的取消 / 续跑 / 重启恢复配方，整跑 **179/179**。素材走画布拖拽导入：
   合成 File 拿不到本机路径，因此命中的是 `importMediaBuffer` 分支，而该 IPC 只收图片/视频——
...
免费可做的三条已并入 §7.9 那条门禁（`npm run test:node-matrix`，配方 `p3`）：

1. ✅ 取消：跑一半点顶栏「停止」→ 节点落定 `cancelled`、未跑项记 `skipped`、不记成功；
2. ✅ 续跑：切「续跑未完成」→ 已成功项 `reused` 不重跑、剩余项补完；
3. ✅ 重启恢复：关掉 Electron 进程再开 → 逐项结果、循环体产物与进度条都从磁盘回来。

仍需授权的一部分：真实成片下载与入库后的下游消费（要一次付费运行），排在付费环节一起做。
