...
语音合成节点的供应商决定动态端口：MiniMax 使用 `in-text`、`in-voice`（接收 `voice.profile@1`）；
火山引擎使用 `in-text`、可选 `in-audio` 参考音频。两种供应商都输出 `out-audio`，只有火山引擎
可选输出 `out-subtitle` (`voice.subtitle@1`)。
