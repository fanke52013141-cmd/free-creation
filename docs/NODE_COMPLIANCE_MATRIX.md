...
本表是当前 **26 个可创建节点** 的发布前协议索引。新增或修改节点前必须同步更新本表、`NODE_CONTRACT_SPEC.md`、契约快照与连线测试；脚本节点仅为历史兼容，不进入创建菜单。表中「版本 / 端口 id」两列由 `test/compliance-matrix-doc.test.ts` 与注册契约逐行比对，写错或漏行会直接让测试变红。
...
| 循环 `iterate`            | 2    | `in-list` json `list.items@1` / one / 必填                                        | `out-item` iteration（循环体）；`out-items` json `list.items@1`   | executor + projectOutputs                | 通过 |
| 导演台 `director`         | 3    | 分镜 JSON、参考图多输入、机位 JSON                                                | 帧、预演视频、机位、工程摘要                                      | manual-publish + projectOutputs          | 通过 |
