...
describe('统一画布视觉基础', () => {
  ...
  it('节点卡片与端口点是磨砂玻璃材质（半透明底 + backdrop 模糊 + 内高光）', () => {
    ...
    expect(legacy).toMatch(/\.node-card\s*\{[\s\S]*?backdrop-filter:\s*blur\(14px\)/)
    expect(legacy).toMatch(/\.node-card\s*\{[\s\S]*?inset 0 1px 0 rgba\(255, 255, 255, 0\.07\)/)
    expect(legacy).not.toMatch(/\.node-card\s*\{[^}]*background:\s*var\(--card\);/)
