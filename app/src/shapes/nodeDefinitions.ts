...
export const NODE_DEFINITIONS: readonly NodeDefinition[] = [
  ...
  { type: 'annotate-edit', name: '标注修图', icon: '✎', desc: '通过标注数据编辑图片', category: 'generate', inputs: [{ name: 'image', kinds: ['Image'] }, { name: 'annotation', kinds: ['Structured'] }], outputs: [output('image', 'Image')], defaultSize: SIZE, implemented: false, color: '#ef4444' },
  { type: TEXT_TYPE, name: '文本资产', icon: 'T', desc: '可复用的纯文本或 Markdown 内容', category: 'asset', inputs: [], outputs: [output('text', 'Text')], defaultSize: SIZE, implemented: true, color: '#f59e0b' },
  { type: IMAGE_ASSET_TYPE, name: '图片资产', icon: '◫', desc: '入库并发布稳定图片引用', category: 'asset', inputs: [], outputs: [output('image', 'Image')], defaultSize: SIZE, implemented: true, color: '#06b6d4' },
  { type: 'video-asset', name: '视频资产', icon: '▻', desc: '入库并发布稳定视频引用', category: 'asset', inputs: [], outputs: [output('video', 'Video')], defaultSize: SIZE, implemented: false, color: '#10b981' },
